...
Reads Hermes hook JSON from stdin, posts a generic Adapter event to ViviPet,
...
Recommended event flow:
  ...
  pre_tool_call         -> tool:start     + generic tool/working state
...
The hook intentionally does not classify tool names such as bash/read/test/edit.
Tool names are preserved in metadata only. ViviPet owns behavior planning,
motion fallback, expressions, props, TTS, and Live2D parameters.
...
Environment overrides:
  ...
  VIVIPET_TTS_ON_MESSAGE       default: 0
...
def classify_event(event_name: str, hook: dict[str, Any]) -> tuple[str, str | None]:
...
def pick_level(hook: dict[str, Any]) -> str | None:
    if hook_has_error(hook):
        return "error"
...
def pick_tts(event_name: str) -> bool | dict[str, bool]:
    if event_name == "post_llm_call" and os.environ.get("VIVIPET_TTS_ON_MESSAGE") == "1":
        return {"enabled": True}
    if event_name in ("on_session_end", "on_session_finalize") and os.environ.get("VIVIPET_TTS_ON_SESSION_END") == "1":
        return {"enabled": True}
    return False


def session_ttl_ms() -> int:
    return int(os.environ.get("VIVIPET_HOOK_SESSION_TTL", "45000"))


def should_refresh_ttl(phase: str) -> bool:
    return phase not in {"session:end", "tool:error", "tool:success"}
...
def build_adapter_payload(hook: dict[str, Any]) -> dict[str, Any]:
    ...
    phase, action = classify_event(event_name, hook)
    ...
    payload: dict[str, Any] = {
        ...
        "level": pick_level(hook),
        "ttlMs": session_ttl_ms() if should_refresh_ttl(phase) else None,
